...
public class RockItem extends BlockItem implements ProjectileItem {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level level, Player player, @NotNull InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            Rock rock = new Rock(level, player);
            rock.setItem(itemstack);
            rock.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.0F, 1.0F);
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, Position pos, @NotNull ItemStack stack, @NotNull Direction direction) {
        Rock rock = new Rock(level, pos.x(), pos.y(), pos.z());
        rock.setItem(stack);
        return rock;
    }
}
